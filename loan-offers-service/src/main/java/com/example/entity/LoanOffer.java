...
public class LoanOffer extends ReactivePanacheMongoEntity {
     ...
     */
    public static Uni<List<LoanOffer>> retrieveLoanOffersThatSumToAtLeastValue(final BigDecimal amountRequested) {
    ...
    /**
     * This function pages through the database, given a specific page size. It queries according to rate, and always
     * returns lower rates first.
     * In terms of operation, it gets a page of results, checks if the amount is enough and if not then retrieves
     * another page of results. Continues until the whole database has been checked. Uses paging in mutiny:
     * https://smallrye.io/smallrye-mutiny/guides/pagination
     *
     * NB: Must be careful to only subscribe to this once in a single request context - else the atomic reference
     * will become overused and not work correctly
     *
     * @param amountRequested the amount that must be fulfilled
     * @return a Multi of loanOffers that either fulfil the loan, or represent all the loans in the database
     */
    private static Multi<LoanOffer> getLoanPagesUntilLoanOfferValue(final BigDecimal amountRequested) {
    ...
    /** This function looks at a list of loan offers and returns the first n that are enough to fulfil the amount
     * requested, or returns a nullItem when fulfilment is not possible.
     *
     * NB: Must be careful to only subscribe to this once in a single request context - else the atomic reference
     * will become overused and not work correctly
     *
     ...
     * @return a list of loans that are sufficient to fulfil the loan, or a nullItem if there are not enough offers
     * to fulfil that amount
     */
    private static Uni<List<LoanOffer>> listLoansRequiredToFulfilRequest(Multi<LoanOffer> loanOffers,
                                                                         final BigDecimal amountRequested) {
        AtomicReference<BigDecimal> currentTotal = new AtomicReference<>(new BigDecimal(0));
        return loanOffers.select().first(offer -> currentTotal.accumulateAndGet(new BigDecimal(offer.amount),
                BigDecimal::add).compareTo(amountRequested) < 0)
                .collect()
                .asList()
                .onItem()
                .transformToUni(offersList -> {
                    if (currentTotal.get().compareTo(amountRequested) < 0) {
                        logger.info("Current total  {}, amount requested: {}", currentTotal.get().toString(),
                                amountRequested.toString());
                        return Uni.createFrom().nullItem();
                    } else {
                        logger.info("Current total return: {}", currentTotal.get().toString());
    ...
    /** A convenience method for summing the amount values in a list of loan offers. Throws if it can't get an
     * element from list.
     *
