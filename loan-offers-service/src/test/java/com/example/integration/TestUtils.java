...
public class TestUtils {
    ...
    static JsonbConfig config =
            new javax.json.bind.JsonbConfig()
    ...
    public static void sendLoanOfferCommand(String id, String rate, String amount) {
        ...
        producerMap
                .get("loan-offers-in")
                .send(new ProducerRecord<>("loan-offers-in", jsonb.toJson(loanOfferCommand)));
    }
    ...
    public static void sendLoanRequestCommand(String id, String amount) {
        ...
        producerMap
                .get("loan-requests-in")
                .send(new ProducerRecord<>("loan-requests-in", jsonb.toJson(loanRequestCommand)));
    }

    public static void assertExistsInDatabase(
            String id, String rate, String amount, int expectedNumberOfDatabaseRecords) {
        await().atMost(Duration.ofMillis(5000))
                .untilAsserted(
                        () -> {
                            MongoCollection<Document> loanOffers =
                                    mongoClient.getDatabase("loans").getCollection("LoanOffer");
                            assertThat(loanOffers.countDocuments())
                                    .isEqualTo(expectedNumberOfDatabaseRecords);
    ...
    public static void assertLoanAvailableMessageInKafka(
            LoanAvailableEvent expected, int expectedNumberOfKafkaMessages) {
        ...
        await().atMost(Duration.ofMillis(10000))
                .untilAsserted(
                        () -> {
                            final ConsumerRecords<String, String> loanAvailableRecords =
                                    loansAvailableConsumer.poll(Duration.ofMillis(1000));
                            assertThat(loanAvailableRecords.count())
                                    .isEqualTo(expectedNumberOfKafkaMessages);
                            List<LoanAvailableEvent> loanAvailableEvents = new ArrayList<>();
                            for (ConsumerRecord<String, String> record : loanAvailableRecords) {
                                loanAvailableEvents.add(
                                        jsonb.fromJson(record.value(), LoanAvailableEvent.class));
                            }
