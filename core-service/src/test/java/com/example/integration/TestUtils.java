...
public class TestUtils {
    ...
    public static void assertLoanOfferMessageInKafka(
            LoanOfferCommand expected, int expectedNumberOfKafkaMessages) {
        ...
        Awaitility.await()
                ...
                .untilAsserted(
                        () -> {
                            final ConsumerRecords<String, String> loanOffersRecords =
                                    loanOffersConsumer.poll(Duration.ofMillis(1000));
                            assertThat(loanOffersRecords.count())
                            ...
                            for (ConsumerRecord<String, String> record : loanOffersRecords) {
                                loanOfferCommands.add(
                                        jsonb.fromJson(record.value(), LoanOfferCommand.class));
                            }
    ...
    public static void assertLoanRequestMessageInKafka(
            LoanRequestCommand expected, int expectedNumberOfKafkaMessages) {
        // Get the appropriate consumer, point to the first message, and pull all messages
        ...
        Awaitility.await()
                ...
                .untilAsserted(
                        () -> {
                            final ConsumerRecords<String, String> loanOffersRecords =
                                    loanRequestsConsumer.poll(Duration.ofMillis(1000));
                            assertThat(loanOffersRecords.count())
                                    .isEqualTo(expectedNumberOfKafkaMessages);
                            List<LoanRequestCommand> loanRequestCommands = new ArrayList<>();
                            for (ConsumerRecord<String, String> record : loanOffersRecords) {
                                loanRequestCommands.add(
                                        jsonb.fromJson(record.value(), LoanRequestCommand.class));
                            }
