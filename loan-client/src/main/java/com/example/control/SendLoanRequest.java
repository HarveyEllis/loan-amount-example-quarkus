...
class SendLoanRequest implements Callable<Integer> {
    ...
    public Integer call() {
        ...
        Multi<LoanAvailableEvent> serverSentLoanEvents =
                Multi.createFrom()
                        .publisher(loanService.getEvents())
                        .map(
                                inboundSseEvent ->
                                        jsonb.fromJson(
                                                inboundSseEvent.readData(),
                                                LoanAvailableEvent.class))
                        .select()
                        .where(
                                loanAvailableEvent ->
                                        loanAvailableEvent.requesterId.equals(borrowerId))
                        .select()
                        .first(1);
        ...
        serverSentLoanEvents.subscribe().with(this::printLoanAvailableOutput);
        return 0;
    }
        ...
        if (loanAvailableEvent.available) {
            System.out.println(
                    "Annual interest rate = "
                            + new BigDecimal(loanAvailableEvent.annualInterestRate)
                                    .multiply(new BigDecimal(100))
                                    .round(twoSf)
                            + "%");
            System.out.println(
                    "Monthly repayment = "
                            + new BigDecimal(loanAvailableEvent.monthlyRepayment)
                                    .setScale(2, RoundingMode.HALF_UP));
            System.out.println(
                    "Total Repayment = "
                            + new BigDecimal(loanAvailableEvent.totalRepayment)
                                    .setScale(2, RoundingMode.HALF_UP));
        }
    }
}
